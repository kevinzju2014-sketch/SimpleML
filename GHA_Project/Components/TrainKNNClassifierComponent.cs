...
{
    public class TrainKNNClassifierComponent : GH_Component
    {
        public TrainKNNClassifierComponent()
          : base(L.Name("TrainKNNClassifierComponent"), L.Nick("TrainKNNClassifierComponent"), L.Desc("TrainKNNClassifierComponent"),
        ...
        protected override void SolveInstance(IGH_DataAccess DA)
        {
            ...
            try
            {
                ...
                jsonBuilder.Append("{");
                jsonBuilder.Append($@"""algorithm"": ""knn""");
        ...
        {
            return @"组件名称: Train KNN Classifier
功能: 配置KNN（K近邻）分类器参数（不进行实际训练）
...
Create Dataset → Split Dataset
  ...
  → Train KNN Classifier → Algorithm Params → Train Classifier (Algorithm输入)
  → Train Classifier → Model

参数调优流程:
Train KNN Classifier (N Neighbors=3) → Algorithm Params A → Train Classifier → Model A
Train KNN Classifier (N Neighbors=5) → Algorithm Params B → Train Classifier → Model B
Train KNN Classifier (N Neighbors=7) → Algorithm Params C → Train Classifier → Model C
(比较不同k值的模型性能)
...
7. 高维数据（>50维）效果可能较差";
        ...
        protected override System.Drawing.Bitmap Icon => IconLoader.LoadComponentIcon(nameof(TrainKNNClassifierComponent));
        public override Guid ComponentGuid => new Guid("F4A5B6C7-D8E9-0123-7890-123456789013");
    }
}
