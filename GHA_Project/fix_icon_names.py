...
# 需要的目标文件名（根据ComponentIconMap.cs）
required_files = {
    # 共享图标（多个组件使用同一个图标）
...
for target_file, source_options in required_files.items():
    ...
    if source_found:
        ...
        print(f'⚠ 未找到 {target_file} 的源文件 (选项: {", ".join(source_options)})')
        missing.append((target_file, source_options))

print('\n' + '=' * 70)
print(f'完成！创建了 {len(created)} 个图标文件')
if missing:
    print(f'\n警告：{len(missing)} 个图标文件未找到源文件')
    for target, options in missing:
        print(f'  - {target} (源文件选项: {", ".join(options)})')
